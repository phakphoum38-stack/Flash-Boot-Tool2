...
export default function App() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      setVerify((v) => v + (rawVerify.current - v) * 0.15);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        if (!window.electron?.getUsbDevices) {
          return;
        }
        ...
        setUsb(Array.isArray(list) ? list : []);
      } catch (err) {
        console.error(err);
      }
    };
  ...
  // =========================
  useEffect(() => {
    if (!window.electron?.onFlashEvent) {
      return;
    }

    const off =
      window.electron.onFlashEvent((ev) => {
